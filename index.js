...
const RiftNode = require('./node.js');
...
class EonRift {
  ...
  spawnNode() {
    ...
    const state = Math.floor(Math.random() * 3); // 0: core, 1: edge, 2: void
    this.nodes.push(new RiftNode(col * this.gridSize, row * this.gridSize, state));
  }
  ...
  checkConnections() {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      for (let j = i + 1; j < this.nodes.length; j++) {
        ...
        if (
          ...
          this.ctx.strokeStyle = n1.state === 0 ? '#ff4d6a' : n1.state === 1 ? '#4d8cff' : '#4dffa6';
    ...
    if (toRemove.length > 0) {
      this.spawnNode();
      if (this.score >= this.stage * 500) this.advanceStage();
    }
  }
  ...
  advanceStage() {
    ...
    this.nodes.forEach(n => (n.flux = Math.min(n.flux + 10, 100)));
    this.spawnNode();
  }
  ...
  drawUI() {
    ...
    this.ctx.fillText(`Stage: ${this.stage}`, 10, 45);
  }
...
const game = new EonRift();
game.update();
console.log('Eon Rift game initialized. Use a UI framework or save canvas to render.');
const fs = require('fs');
